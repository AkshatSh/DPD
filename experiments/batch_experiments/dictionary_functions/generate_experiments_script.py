    ...
    for trial in range(NUM_TRIALS):
        for weak_weight in WEAK_WEIGHT:
            for weak_function in WEAK_FUNCTIONS:
                ...
                command = f'python dpd/allennlp_active_train.py --weak_function {weak_function} --num_epochs {NUM_EPOCHS} --weak_weight {weak_weight} --model_name {model_name} --use_weak --cuda'
...
def main():
    ...
    bash_script = os.path.join(os.path.dirname(__file__), 'weak_active_model_experiments.sh')
